...
#    # Bounds on HSV Colour Space (Initial Bound)
...
#    bound2 = np.array([[144, 25, 127], [180, 255, 255]])
    ...
    # Bounds on HSV Colour Space
    bound1 = np.array([[0, 0.31, 0.26], [0.12, 1.0, 0.73]])
    bound2 = np.array([[0.89, 0.31, 0.26], [1.0, 1.0, 0.73]])
...
img_hsv = rgb2hsv(rescale(imread('Perching Target/target_1.jpg'), 0.5))
...
if opt_plot:
    ...
    ax[2].imshow(mask, cmap = cm.gray)
    ax[2].set_title('Boolean Mask')
    ax[2].axis('off')
...
lines_pos, grad_pos, orients_pos = run_hough_transform(trg_region.filled_image, (0, 90, 45))
lines_neg, grad_neg, orients_neg = run_hough_transform(trg_region.filled_image, (-90, 0, 45))
...
if opt_plot:
    ...
    ax[0].imshow(trg_region.image, cmap = cm.gray)
    ...
    ax[1].imshow(trg_region.filled_image, cmap = cm.gray)
    ax[1].set_title('Filled Target')
    ax[1].axis('off')
